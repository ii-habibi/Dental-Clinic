...
router.post('/delete/:id', async (req, res) => {
    const doctorId = req.params.id;
    try {
        // Step 1: Get the doctor's details to retrieve the image path
        const result = await pool.query('SELECT photo FROM doctors WHERE id = $1', [doctorId]);
        const doctor = result.rows[0];

        // Step 2: Delete the doctor profile from the database
        await pool.query('DELETE FROM doctors WHERE id = $1', [doctorId]);

        // Step 3: Delete the image file from the server
        if (doctor && doctor.photo) {
            const imagePath = path.join(__dirname, '../public/', doctor.photo); // Ensure you get the correct path
